...
**First-run wall time is ~150 seconds for an active brand.** The CLI runs four discovery passes (user search → hashtags → campaign codes → seed-creator walk) plus repeat-queries each hashtag until saturation, to work around TikTok's per-hashtag result cap. Tell the user this is expected before running, especially if they're impatient. Subsequent searches on the same brand serve from cache instantly. Use `--refresh` to force a re-fetch (also takes ~150s).
