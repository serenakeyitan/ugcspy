...
- "What is @glossier posting on TikTok?" → `ugcspy search @glossier`
- "Track Rare Beauty's organic content for me" → `ugcspy watch add @rarebeauty --slack-webhook ...`
- "Turn this video into a brief I can hand to a creator" → `ugcspy fork <url>`
- Any request that maps to "BigSpy but for organic posts."
...
ugcspy search <handle> [--platform tiktok|instagram|all] [--limit N] \
                      [--sort engagement|recency] [--format <tags>] [--json]
```

Returns the handle's recent organic videos ranked by engagement (default) or recency, with extracted hooks and format tags. Use `--json` when you need to feed results into the rest of a workflow.
...
ugcspy daemon --once       # poll all watches once and post Slack alerts
ugcspy daemon              # loop every 6h (default)
```

Cold-start gate: alerts stay in `warming_up` for 7 days OR until ≥5 videos exist in the trailing window — whichever takes longer. Don't expect alerts on day 1.
