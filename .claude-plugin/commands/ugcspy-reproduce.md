...
- **AI-disclosure is opt-in**: by default the reproduction.mp4 carries NO watermark and compose prints a one-line reminder that the output is unlabeled. Pass `--disclosure` to burn a small "AI-generated" tag (bottom-right); customize with `--disclosure-text` / `--disclosure-position`. FTC + EU AI Act + platform ToS still require labeling AIGC before posting publicly — enable it (or label downstream) when you publish.
...
- **AI-disclosure watermark is opt-in (off by default).** Pass `--disclosure` to burn a small "AI-generated" tag bottom-right (configurable via `--disclosure-text` and `--disclosure-position`). When omitted, the reproduction is unlabeled and compose prints a reminder. FTC + EU AI Act + TikTok ToS require AIGC labeling — enable the watermark or apply a label downstream before posting publicly.
